...
CHECK_STATUS() {
    ...
    POD_NAME=${5:-$3}
    ADD_COMMAND=${6:-""}
    GET_HELM_NAME=$(${CLI} -A | grep ${HELM_NAME} | awk '{print $2}' | tail -1 )
    ## -z : null 일때 참
    if [ -z "${GET_HELM_NAME}" ]
    then
        bash ${SHELL_PATH}
...
CHECK_COMMAND platform jenkins ${HELM_PATH}/jenkins jenkins-install.sh "bash ${ETC_PATH}/jenkins-image-push.sh"
